...
public class Fracture : MonoBehaviour, IShootable
{
    ...
    [SerializeField][MinValue(0.0f)] private float _impactForce = 50.0f;
    [SerializeField][MinValue(0.0f)] private float _impactRadius = 15f;
    [SerializeField] private bool _canBeShot = true;
    ...
    /// </summary>
    /// <returns></returns>
    private List<GameObject> ComputeFracture(Vector3? worldHitPoint = null)
    {
        var mesh = this.GetComponent<MeshFilter>().sharedMesh;
        List<GameObject> fragmentCollection = new();
        ...
        if (mesh != null)
        {
            ...
            if (worldHitPoint.HasValue)
                localHitPoint = this.transform.InverseTransformPoint(worldHitPoint.Value);
            ...
            {
                fragmentCollection = Fragmenter.Fracture(this.gameObject,
                                                        this.fractureOptions,
                                                        fragmentTemplate,
                                                        this.fragmentRoot.transform,
                                                        false,
    ...
    private GameObject CreateFragmentTemplate()
    {
        ...
        if (fractureOptions.insideMaterial != null)
            sliceMaterial = fractureOptions.insideMaterial;
        else
            sliceMaterial = this.GetComponent<MeshRenderer>().sharedMaterial;
    ...
    private void ApplyForce(GameObject fragment, Vector3 origin, float radius, float strength)
    {
        if (!fragment.TryGetComponent<Rigidbody>(out Rigidbody rb))
            return;
    ...
    public void Hit(float dmg, Vector3 point)
    {
        ...
        List<GameObject> fragmentCollection = ComputeFracture(point);

        foreach (GameObject fragment in fragmentCollection)
        {
            ApplyForce(fragment, point, _impactRadius, _impactForce);
        }
    }
}
