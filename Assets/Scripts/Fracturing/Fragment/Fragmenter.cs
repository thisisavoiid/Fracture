...
public static class Fragmenter
{
                                ...
                                Vector3? localHitPoint = null)
    {
        ...
        while (fragments.Count < options.fragmentCount)
        {
            ...
            Vector3 sliceOrigin = meshData.Bounds.center;

            if (localHitPoint.HasValue)
            {
                if (meshData.Bounds.Contains(localHitPoint.Value))
                {
                    sliceOrigin = Vector3.Lerp(
        ...
        {
            List<GameObject> fragmentSet = CreateFragment(meshData,
                           sourceObject,
                           fragmentTemplate,
                           parent,
                           saveToDisk,
            ...
            foreach (var item in fragmentSet)
            {
                item.layer = options.fragmentLayer;
                fragmentCollection.Add(item);
            }
